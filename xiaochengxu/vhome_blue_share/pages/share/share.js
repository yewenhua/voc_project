...
Page({
    ...
    onLoad: function (options) {
        ...
        app.wechatlogin(function () {
            that.setData({
                ...
                deviceKeyId: options.did ? options.did : ''
            });

            if (that.data.modelName != '' && that.data.deviceKeyId != '') {
                that.setData({
                    hasInit: true
                });
    ...
    getOpenDirective: function () {
        ...
        wx.request({
            method: 'post',
            url: app.globalData.wxurl + '/api/directive',
            header: {
                'Content-Type': 'application/json'
            },
            data: {
                deviceKeyId: that.data.deviceKeyId,
                command: 'OPEN_WITH_KEY',
                directive: that.data.randomCode,
                third_session: app.globalData.third_session.value
            },
            success: function (res) {
                if (res.data.hasOwnProperty('type') && res.data.type == 'SUCCESS') {
                    that.setData({
    ...
    sendResult: function () {
        ...
        wx.request({
            method: 'post',
            url: app.globalData.wxurl + '/api/sendresult',
            header: {
                'Content-Type': 'application/json'
            },
            data: {
